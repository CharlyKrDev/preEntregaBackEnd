...
productsRouter.get("/api/products", async (req, res) => {
    ...
    productsRouter.get('/api/products/:pid', async (req, res) => {
        ...
        try {
            ...
            if (!product) return res.status(404).json(`El id: ${id} no pertenece a ningún producto`);
    ...
    productsRouter.post('/api/products', async (req, res) => {
        ...
        try {
            ...
            if (!(title && description && price && code && stock && category)) {
                return res.status(400).json({ error: 'Las siguientes key son obligatorios: title, description, code, price, status, stock, category' });

            }

            if (product.status !== true && product.status !== false && product.status !== undefined) {

                return res.status(400).json({ error: 'Status solo puede ser true o false, si no se carga, por defecto es true' })
            }
            ...
            res.status(200).json({ Producto: product, message: `Producto cargado correctamente` })
    ...
    productsRouter.put('/api/products/:pid', async (req, res) => {
        ...
        try {
            ...
            if (!checkId) {
                ...
                return res.status(400).json({ message: `El ID no corresponde a ningún producto` })
            }
            if (product.id) {
                return res.status(400).json({ message: `El ID no se puede cambiar, las keys son: title, description, code, price, status, stock, category y thumbnail` })
            };
            ...
            res.status(201).json({ ProductoID: `${id}`, message: `Producto actualizado correctamente` })
    ...
    productsRouter.delete('/api/products/:pid', async (req, res) => {
        ...
        try {
            ...
            res.status(200).json(`El producto id: ${productId} ha sido eliminado correctamente`);
