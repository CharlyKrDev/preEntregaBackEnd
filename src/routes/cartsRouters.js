...
cartsRouter.get('/api/carts/:cid', async (req, res) => {
    ...
    try {
        ...
        if (!cart) return res.status(404).json(`El id: ${id} no pertenece a ningún carrito`);
...
cartsRouter.post("/api/carts", async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ Carrito: cartEmpty, message: `Creado correctamente` })
...
cartsRouter.post("/api/carts/:cid/products/:pid", async (req, res) => {
    ...
    try {
        ...
        if (!checkId) {
            return res.status(400).json(`Debe seleccionar un id de producto existente`);
      
        }
        if (!checkIdCart) {
            return res.status(400).json(`Debe seleccionar un id de un carrito existente`);
        }
        const addToCart = cartsManager.addProductToCart(cartId,prodId);

        res.status(201).json({message: `Agregado producto id: ${prodId} al carrito id: ${cartId}`});
    } catch (error) {
        res.status(500).json(`Error al obtener cargar producto en carrito`, error);
    }
});
